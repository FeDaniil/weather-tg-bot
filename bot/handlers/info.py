...
@router.callback_query(MagicData(F.event.data == "info"))
async def info_cb_handler(query: types.CallbackQuery) -> None:
    """Information about bot."""
    df = px.data.wind()
    fig = px.bar_polar(df, r="frequency", theta="direction",
                   color="strength", template="plotly_dark",
    ...
    img_bytes = fig.to_image(format="png")
    # test reply with image
    await query.message.answer_photo(BufferedInputFile(img_bytes, filename="plot.png"), caption=_("about"))